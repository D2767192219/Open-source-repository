...
int thread01()
{
    ...
    */
    ...
    bbox = boundingRect(maxAreaContour);
    ...
    while (video.read(frame))
    {
        ...
        // Exit if ESC pressed.
        circle(frame,Point(bbox.x+bbox.width/2,bbox.y+bbox.height/2),5,Scalar(230,130,255),2,8);
        cout<< bbox.x+bbox.width/2<<"\t"<<bbox.y+bbox.height/2<<endl;
        imshow("Tracking", frame);
        int k = waitKey(30); // Display frame.
        if (k == 27)
        {
            break;
        }
...
{   
    ...
    {   
        ...
        String xxx=to_string(bbox.x+bbox.width/2);
        String yyy=" "+to_string(bbox.y+bbox.height/2);
        String point=xxx+yyy;
        //int y=sizeof(xxx+yyy);
        char recvBuf[20];
        strncpy(recvBuf,point.c_str(),point.length()+1);
        write(sp,buffer(recvBuf,20));
        iosev.run();
        sleep(1);
        int i=0;
        i++;
